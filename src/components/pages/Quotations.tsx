...
export function Quotations({ onNavigate }: { onNavigate?: (p: Page) => void }) {
  const { quotations, clients, refresh } = useData();
  ...
  const convertToInvoice = async (q: Quotation) => {
    ...
    if (error || !inv) { console.error('[Supabase] Failed to convert quotation:', error); toast({ title: 'Failed to convert', description: error?.message || 'Unknown error', variant: 'destructive' }); return; }
...
function QuotationBuilder({ onClose }: { onClose: () => void }) {
  ...
  const save = async () => {
    ...
    if (error || !quote) { console.error('[Supabase] Failed to save quotation:', error); toast({ title: 'Failed to save quotation', description: error?.message || 'Unknown error', variant: 'destructive' }); return; }
    const validItems = items.filter((it) => it.description.trim());
    if (validItems.length > 0) {
      await supabase.from('quotation_items').insert(validItems.map((it) => ({
