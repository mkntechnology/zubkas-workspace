...
export function Invoices() {
  const { invoices, clients, payments, refresh } = useData();
...
function InvoiceBuilder({ editing, onClose }: { editing: Invoice | null; onClose: () => void }) {
  ...
  const save = async () => {
    ...
    if (editing) {
      ...
      if (error || !inv) { console.error('[Supabase] Failed to create invoice:', error); toast({ title: 'Failed to create invoice', description: error?.message || 'Unknown error', variant: 'destructive' }); return; }
      if (validItems.length > 0) await supabase.from('invoice_items').insert(validItems.map((it) => ({ invoice_id: inv.id, description: it.description, quantity: it.quantity, rate: it.rate, amount: it.amount })));
