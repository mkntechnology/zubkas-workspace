...
import { Plus, Users, Mail, Phone, Building2, MapPin, Pencil, Trash2, X, FileText, Receipt, Wallet } from 'lucide-react';
...
export function Clients() {
  const { clients, invoices, payments, refresh } = useData();
  ...
  const save = async () => {
    ...
    if (editing) {
      ...
      if (error) { console.error('[Supabase] Failed to create client:', error); toast({ title: 'Failed to create client', description: error.message, variant: 'destructive' }); return; }
